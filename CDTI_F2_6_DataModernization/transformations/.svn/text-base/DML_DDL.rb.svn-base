...
decorator DDL::Pk do
  def getColumnNames
    ...
    i=0
    self.columnName.each do
      columnNames[i] = self.columnName[i].strip
      columnNames[i].strip
      i = i + 1
    end
    return columnNames
...
rule 'Pk2Pk' do
  ...
  mapping do |ddl, dml_ddl|
    ...
    #dml_ddl.columnName = ddl.getColumnNames
...
rule 'Registry2Registry' do
  ...
  mapping do |dml, dml_ddl|
    puts "COUNTER: #{Contador.next}"
    dml_ddl.registryValues = dml.registryValues
    #print "<"
    #dml_ddl.registryValues.each do |val|
    #  print "#{val.value},"
    #end
    #puts ">"
  end
...
rule 'Value2Value' do
  ...
  mapping do |dml, dml_ddl|
    ...
    #print "VALUE: #{dml.value}"
    if (dml.column == nil)
      puts "<<<<<<<<<<<<<<<<<ERROR NIL ERROR NIL ERROR NIL ERROR NIL >>>>>>>>>>>>>>>>>>>>"
    end
    #print " --> COLUMN: #{dml.column.columnName}"
