...
public class DetectionErrorsMainPage extends WizardPage {
	...
	private void dialogChanged() {
		...
		if (dotLoc != -1) {
			...
			if (ext.equalsIgnoreCase("ecore") == false && ext.equalsIgnoreCase("xmi") == false) {
				updateStatus("Model DML extension must be \"ecore\" or \"xmi\"");
				return;
			}
		}
		...
		if (dotLoc != -1) {
			...
			if (ext.equalsIgnoreCase("ecore") == false && ext.equalsIgnoreCase("xmi") == false) {
				updateStatus("Model DDL extension must be \"ecore\" or \"xmi\"");
				return;
			}
		}
