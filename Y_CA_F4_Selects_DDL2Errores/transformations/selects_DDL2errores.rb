decorator SEL::Fichero do
  def obtenerTablaYColumnaDDL(operando, tabs)
    ...
    if tablaOAlias!=""
      tabla=tabs.find{|t| t.nombre.upcase == tablaOAlias.upcase || t.tabAlias.upcase == tablaOAlias.upcase}
      if tabla!=nil
    ...
    if(tablaNombre!="")
      ...
      tabs.each do |tab|
        ...
        tabla = DDL::Table.all_objects.find{|t| t.name.upcase == tab.nombre.upcase}
        
        if tabla != nil 
          resultado[1]= tabla.columns.find{|c| c.name.upcase == columnaNombre.upcase}
        end
  ...
  def obtenerCandidatos(tabYcol1, tabYcol2)
   ...
   if tabYcol1[0] != nil && tabYcol2[0] != nil && tabYcol1[1] != nil && tabYcol2[1] != nil     
    ...
    if tab1PK != nil && tab1PK.columnName.any?{|cn| cn.strip.upcase == tabYcol1[1].name.upcase}
      # La primera columna pertenece a la pk
      # Comprobamos si la segunda columna pertenece ya a una fk
      if !tab2FKs.any?{|fk| fk.columnName.strip.upcase.include?(tabYcol2[1].name.upcase)}
    ...
    if tab2PK != nil && tab2PK.columnName.any?{|cn| cn.strip.upcase == tabYcol2[1].name.upcase}
      # La segunda columna pertenece a la pk
      # Comprobamos si la primera columna pertenece ya a una fk
      if !tab1FKs.any?{|fk| fk.columnName.strip.upcase.include?(tabYcol1[1].name.upcase)}
        ...
        if columnasPkTab1.size==1 # Comprobamos si la columna referenciada es la unica de la PK
          ...
          columnasRestantesPkTab1.each do |crpk|
            ...
            while j<tamCandidatos && !indicadorParcial do
              if j!=i
                cand2=candidatos[j]
                if cand2[4]=="false" && cand2[3].name.upcase == crpk.upcase && cand[0].name.upcase == cand2[0].name.upcase
...
top_rule 'fichero2errores' do
  ...
  mapping do |source, target|
    ...
    #if fichero.nombre == '/FrmMMSERecord.selects'
      # puts 'ENCONTRADO: ' + fichero.nombre
      ...
      sels.each do |s|
       if s.parte_where != nil && s.parte_from != nil 
        ...
        while joins.size>0 do
          ...
          if tablaYColumnaDDL1[0] != nil && tablaYColumnaDDL1[1] != nil && tablaYColumnaDDL2[0] != nil && tablaYColumnaDDL2[1] != nil
            puts '(1)' + fichero.nombre + ' , ' + tablaYColumnaDDL1[0].name + '.' + tablaYColumnaDDL1[1].name + ' , ' + tablaYColumnaDDL2[0].name + '.' + tablaYColumnaDDL2[1].name
          end  
          ...
          if joinCandidato.length>0
            ...
            puts '(2)' + fichero.nombre + ' , ' + joinCandidato[2].name + '.' + joinCandidato[3].name + ' , ' + joinCandidato[0].name + '.' + joinCandidato[1].name
        ...
        while i<tamSolucion do
          if solucion[i][4]=="false"
            target.errores=ERR::ForeignError.new do |fe|
              fe.set('tableCont', DDL::Table.all_objects.find{|t| t.name.upcase == solucion[i][0].name.upcase})
              fe.set('tableRef', DDL::Table.all_objects.find{|t| t.name.upcase == solucion[i][2].name.upcase})
              fe.set('fkColumns', DDL::Table.all_objects.find{|t| t.name.upcase == solucion[i][0].name.upcase}.columns.find{|c| c.name.upcase == solucion[i][1].name.upcase})
              solucion[i][4]="true"
              j=0
              while j<tamSolucion do
                if solucion[j][4]=="false" && solucion[j][0]==solucion[i][0] && solucion[j][2]==solucion[i][2]
                  fe.set('fkColumns', DDL::Table.all_objects.find{|t| t.name.upcase == solucion[j][0].name.upcase}.columns.find{|c| c.name.upcase==solucion[j][1].name.upcase})
                  solucion[j][4]="true"
                end 
                j+=1
              end
